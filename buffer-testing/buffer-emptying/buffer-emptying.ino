...
void setup() {
  // starting native port
  SerialUSB.begin(9600);
  // starting programming port
  Serial.begin(9600);
  // I2C bullshit
  debugSetup();
  // fill all the buffer length to frame size of each buffer
  int i;
  for ( i = 0 ; i < 40 ; i ++ )
  {
    ax25_callsign_frame_length[i] = FRAME_SIZE;
  }
...
void loop() {
  ...
  Serial.print("The time difference for sending frame size ");
